...
def create_dummy_data():
    ...
    for num in range(162):
        ...
        # 농장 규모 500~10000평
        farm_scale = random.randint(500, 10000)
        # 50년 전부터 1년 전까지 날짜 랜덤 생성(timestamp 형태)
        farm_started_time = fake.date_time_between(start_date='-50y', end_date='-1y').strftime('%Y-%m-%d %H:%M:%S')
        # 원두 이름 더미 12개
        farm_bean_name = random.choice(["브라질 산토스", "자메이카 블루마운틴", "콜롬비아 수프리모", "에티오피아 예가체프", "케냐 AA", "코스타리카 따라주", "탄자니아 AA", "예멘 모카 마타리", "하와이 코나", "과테말라 안티구아", "파나마 게이샤", "엘살바도르"])
